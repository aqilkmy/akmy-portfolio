...
const Crosshair: React.FC<CrosshairProps> = ({
  ...
  useEffect(() => {
    if (!visible) return; // ⛔ Stop setup if not visible
    ...
    const enter = () => { if (visible) tl.restart(); };
  ...
  return (
    ...
    >
      ...
      <div
        ...
        className="absolute w-full h-px pointer-events-none transform translate-y-1/2 transition-opacity duration-300 z-[999]"
        style={{ background: color }}
      ></div>
      <div
        ref={lineVerticalRef}
        className="absolute h-full w-px pointer-events-none transform translate-x-1/2 transition-opacity duration-300 z-[999]"
        style={{ background: color }}
      ></div>
    </div>
  );
